...
| 13 | `ControlFlowDeobfuscator` | `dispatcher-deobfuscation` + `control-flow-completion`, asked again late as `dispatcher-recheck` + `control-flow-recompletion` | Clean-room. Proves dispatcher edges, folds constant branches, and deletes all unreachable code with EH regions preserved and per-method verification. Both run twice: a dispatcher only looks like one once the proxy, string and loader calls are out of the block, and a predicate only folds once the state it reads has been recovered. The late dispatcher run then repeats until a round finds nothing, because redirecting an edge shortens the block it left and so makes the next dispatcher recognizable; on three real libraries the rounds after the first account for a third of the edges. |
