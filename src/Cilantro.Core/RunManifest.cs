...
/// Which protector the run settled on, by the name a program uses, or <c>none</c>. Said outright
/// because more than one is recognized and the capability list alone does not tell two of them
/// apart: both name a capability anti-tamper and mean different mechanisms by it.
/// </param>
    ...
    bool MoreToDeclare)
{
    ...
    /// </summary>
    public const string Current = "cilantro.run/1";
...
public sealed record RunFailure(string Error)
{
    ...
    public const string Current = "cilantro.error/1";
