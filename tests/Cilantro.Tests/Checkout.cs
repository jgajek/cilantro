...
{
    /// <summary>The directory holding Cilantro.slnx.</summary>
    ...
    private static string FindRoot()
    {
        ...
        {
            if (File.Exists(Path.Combine(current.FullName, "Cilantro.slnx")))
                return current.FullName;
            current = current.Parent;
        }

        throw new DirectoryNotFoundException(
            $"No Cilantro.slnx above {AppContext.BaseDirectory}.");
    }
}
...
/// <code>dotnet test Cilantro.slnx -c Release --filter "Cost!=High"</code>
